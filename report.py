...
cur.execute('SELECT sum(words), SUM(words) / (STRFTIME("%d") + 1) from wordcount WHERE strftime("%Y-%m", date, "unixepoch", "localtime") = ?', [month])
monthly = cur.fetchone()
if monthly[0] is None:
    monthly = (0, 0)
week = time.strftime("%Y-%W")
cur.execute('SELECT sum(words), SUM(words) / (STRFTIME("%w") + 1) from wordcount WHERE strftime("%Y-%W", date, "unixepoch", "localtime") = ?', [week])
weekly = cur.fetchone()
if weekly[0] is None:
    weekly = (0, 0)
